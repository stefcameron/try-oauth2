...
// - client_id:String ID of the client to whom the auth code was granted
// - client_secret:String Client secret code issue upon client registration/creation
router.post('/', (req, res, next) => {
  const grantType = req.body.grant_type || undefined;
  const clientId = req.body.client_id || undefined;
  const clientSecret = req.body.client_secret || undefined;
  ...
  if (grantType === grantKind.AUTH_CODE) {
    AuthCodeModel.findOne({code: code}, (err, authCode) => {
      ...
      if (!authCode) {
        debug('invalid auth code provided (may have expired): no access');
        res.status(400).end();
        return;
      }
            ...
            refreshToken.save((err) => {
              ...
              token.save((err) => {
                ...
                const payload = {
                  ...
                  expiresIn: token.expiresIn, // {number}
