...
/** @const {number} Time To Live (until expiry) in seconds */
...
const schema = mongoose.Schema({
  ...
  // time, in ms, when this access token will expire, requiring a refresh token handshake
  ...
  expiresAt: {type: Date, default: () => new Date(Date.now() + (TTL * 1000))},
